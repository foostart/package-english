<?php namespace Foostart\English\Controllers\Admin;
...
class EnglishAdminController extends FooController
{
    ...
    public function __construct(Request $request)
    {
        ...
        //validators
        $this->obj_validator = new EnglishValidator();

        //set language files
        $this->plang_admin = 'english-admin';
        $this->plang_front = 'english-front';

        //package name
        $this->package_name = 'package-english';
        $this->package_base_name = 'english';

        //root routers
        $this->root_router = 'english';
        ...
        $this->category_ref_name = 'admin/english';
    ...
    public function config(Request $request)
    {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $config_path = realpath(base_path('config/package-english.php'));
        $package_path = realpath(base_path('vendor/foostart/package-english'));
        ...
        if ($request->isMethod('english') && $is_valid_request) {

            //create backup of current config
            file_put_contents($config_bakup . '/package-english-' . date('YmdHis', time()) . '.php', $content);
    ...
    public function lang(Request $request)
    {
        ...
        $is_valid_request = $this->isValidRequest($request);
        // display view
        $langs = config('package-english.langs');
        $lang_paths = [];
        $package_path = realpath(base_path('vendor/foostart/package-english'));

        if (!empty($langs) && is_array($langs)) {
            foreach ($langs as $key => $value) {
                $lang_paths[$key] = realpath(base_path('resources/lang/' . $key . '/english-admin.php'));
        ...
        if ($request->isMethod('english') && $is_valid_request) {
            ...
            foreach ($lang_paths as $key => $value) {
                ...
                //format file name english-admin-YmdHis.php
                file_put_contents($lang_bakup . '/' . $key . '/english-admin-' . date('YmdHis', time()) . '.php', $content);
            }
